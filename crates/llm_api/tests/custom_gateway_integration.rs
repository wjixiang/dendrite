...
async fn test_basic_message_creation() {
    ...
    match response {
        Ok(msg) => {
            ...
            assert!(msg.usage.as_ref().map(|u| u.input_tokens > 0).unwrap_or(false), "Should have input tokens");
            assert!(msg.usage.as_ref().map(|u| u.output_tokens > 0).unwrap_or(false), "Should have output tokens");

            let text = extract_text_from_content(&msg.content);
            println!("✅ Response: {}", text);
            if let Some(ref usage) = msg.usage {
                println!("📊 Usage: {} input, {} output tokens", usage.input_tokens, usage.output_tokens);
            }
        }
...
async fn test_max_tokens_limits() {
    ...
    assert!(response.usage.as_ref().map(|u| u.output_tokens <= 20).unwrap_or(false), "Should respect max_tokens limit");
}
...
async fn test_comprehensive_feature_set() {
    ...
    assert!(response.usage.as_ref().map(|u| u.input_tokens > 0).unwrap_or(false), "Should have input tokens");
    assert!(response.usage.as_ref().map(|u| u.output_tokens > 0).unwrap_or(false), "Should have output tokens");
