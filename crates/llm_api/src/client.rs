...
impl Anthropic {
    ...
    /// use llm_api::{Anthropic, types::MessageCreateBuilder};
    ///
    ...
    /// ```rust,no_run
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// use llm_api::{Anthropic, types::{BatchRequest, BatchCreateParams}};
    ///
    /// let client = Anthropic::from_env()?;
    ///
    /// let requests = vec![
    ...
    /// ```rust,no_run
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// use llm_api::{Anthropic, FileUploadParams, FilePurpose};
    ///
    /// let client = Anthropic::from_env()?;
    ///
    ...
    /// ```rust,no_run
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// use llm_api::{Anthropic, ModelListParams};
    ///
    /// let client = Anthropic::from_env()?;
    ///
    /// // List all models
