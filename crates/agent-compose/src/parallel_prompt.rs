...
pub const PARALLEL_COMPOSE_PROMPT: &str = concat!(
    ...
    "- 你**不**需要导航到任何具体子树——所有读取都用 `kms_local('/...')` 配绝对路径。\n\n",
    "### 你**做**的工作（两步）\n",
    "1. **分析用户输入，生成拆分计划**。\n",
    ...
    "4. （可选）用 `kms_local('/<候选领域>')` 检查现有树结构，确认拆分是否合理。\n\n",
    "**第二步：生成拆分计划**\n",
    ...
    "2. 并行启动子 Agent（每个 Agent 在自己的 staging area 中工作，用 `kms_local` 配合绝对路径）。\n",
    "3. 等待所有子 Agent 完成。\n",
    ...
    "（可选）合并完成后用 `kms_local('/<target_parent>')` 抽样验证主树结构。\n\n",
    "## 何时不应使用并行模式\n",
